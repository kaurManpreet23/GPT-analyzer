...
class GPTHelper:
    ...
    def _send_request(self, prompt):
        ...
        try:
            ...
            try:
                ...
                return {"status": "success", "format": "json", "data": json_response}
            except json.JSONDecodeError:
                print("Failed to parse OpenRouter response as JSON, formatting as text")
                return {
                    "status": "success",
                    "format": "text",
                    "data": {"content": response_text, "sections": [s.strip() for s in response_text.split('\n\n') if s.strip()]}
                }
    ...
    def analyze_threat(self, query, context=""):
        ...
        Additional Details:
        - **Reason for Attack:** Why was this attack carried out?
        - **Impact:** What was the effect of the attack?
        - **Threat Actor:** Who was responsible for this attack?
        ...
        if isinstance(response, dict) and response.get('status') == 'success':
            ...
            return {"threat_analysis": content}

        return {"error": "Failed to retrieve threat analysis."}
    ...
    def tag_threat_data(self, data):
        ...
        IMPORTANT: Ensure your response is valid JSON that can be parsed with json.loads(). 
        Do not include markdown, backticks, or any text outside of the JSON structure.
        """

        response = self._send_request(prompt)

        # Ensure response contains valid JSON data
        if isinstance(response, dict) and response.get('status') == 'success':
            try:
                json_response = response.get('data', {})
                return json_response  # Returns properly formatted JSON
            except Exception as e:
                print(f"Error parsing JSON response: {str(e)}")
                return {"error": "Failed to process threat tagging."}

        return {"error": "Failed to retrieve tagged threat data."}
